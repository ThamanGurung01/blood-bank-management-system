...
import { calculateNextEligibleDate } from "@/utils/calculateNextEligibleDate";

export const insertBloodDonation = async (
  formData: FormData,
  bloodDonationType: string
) => {
  try {
    ...
    if (
      bloodDonationType !== "new_blood_donation" &&
      bloodDonationType !== "existing_blood_donation"
    )
      return { success: false, message: "Blood donation type is invalid" };
    await connectToDb();
    ...
    if (!errors) {
      ...
      if (bloodDonationType === "new_blood_donation") {
        const bloodDonationData = formDataDeform(
          formData,
          "new_blood_donation"
        ) as IBLood_Donation | undefined;
        if (!bloodDonationType) {
          return { success: false, message: "Donation data is invalid" };
        }
        const cBloodDonation = await BloodDonation.create({
          ...bloodDonationData,
          blood_bank: bloodBankId,
        });
        const bloodExpiryDate = calculateExpiry(
          bloodDonationData?.donation_type,
          bloodDonationData?.collected_date
        );
        const cBlood = await Blood.create({
          ...bloodDonationData,
          blood_bank: bloodBankId,
          expiry_date: bloodExpiryDate,
        });
        return {
          success: true,
          message: `Blood donation successfully created`,
        };
        ...
        const newBloodDonationData = {
          ...bloodDonationData,
          donor_name: existingDonor.user.name,
          donor_contact: existingDonor.contact,
        };
        const cBloodDonation = await BloodDonation.create({
          ...newBloodDonationData,
          blood_bank: bloodBankId,
          donor_type: "existing",
        });
        console.log(
          "blood donation data" + JSON.stringify(newBloodDonationData)
        );

        const bloodExpiryDate = calculateExpiry(
          newBloodDonationData.donation_type,
          newBloodDonationData.collected_date
        );
        const cBlood = await Blood.create({
          ...newBloodDonationData,
          blood_bank: bloodBankId,
          donor: existingDonor._id,
          expiry_date: bloodExpiryDate,
        });
        ...
        const lastDonationDate = existingDonor.last_donation_date
          ? new Date(existingDonor.last_donation_date)
          : null;
        const latestDate =
          !lastDonationDate || blood_collected_date >= lastDonationDate
            ? blood_collected_date
            : lastDonationDate;

        const nextEligibleDate = calculateNextEligibleDate(
          blood_collected_date,
          cBloodDonation.donation_type
        );

        const updated_donated_volume =
          existingDonor.donated_volume +
          Number(newBloodDonationData.blood_units);
        const updatedDonorData = await Donor.findByIdAndUpdate(
          existingDonor._id,
          {
            last_donation_date: latestDate,
            donated_volume: updated_donated_volume,
            score: donorScore,
...
export const getBloodStock = async (bloodType: string) => {
  try {
    ...
    const bloodStock = await Blood.find({
      blood_bank: bloodBankId,
      blood_type: bloodType,
      status: "available",
    }).sort({ createdAt: -1 });
    if (!bloodStock)
      return { success: false, message: "Blood stock not found" };
...
export const getBloodDonations = async (id: string) => {
  try {
    ...
    await connectToDb();
    const donorData = await Donor.findOne({ _id: id }).populate({
      path: "user",
      select: "name email role",
    });
    if (!donorData) return { success: false, message: "Donor not found" };
    const bloodDonationData = await BloodDonation.find({
    ...
    return {
      success: true,
      data: JSON.parse(JSON.stringify(bloodDonationData)),
    };
  } catch (error: any) {
    console.log(error?.message);
    return { success: false, message: "Something went wrong" };
